...
try {
  ...
  if ($action === 'get') {
    ...
    if (!$row) {
      ...
      if (empty($row['favicon_url']) || trim($row['favicon_url']) === '') {
  ...
  if ($action === 'save' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // site_name: free-text display name override, trimmed/length-capped; empty means "use account name"
    $snRaw = trim((string)($data['site_name'] ?? ''));
    $snRaw = str_replace(["\0", "\r", "\n", "\t"], '', $snRaw);
