...
<div class="container">
    ...
    <?php else: ?>
        ...
        <?php if ($tracking && in_array($tracking['delivery_status'], ['Assigned','Picked_Up','In_Transit','Delivered'])): ?>
        <div class="card">
            ...
            $currentIdx = array_search($tracking['delivery_status'], $allStatuses);
            ...
            <div id="trackMap" style="<?php echo ($tracking && $tracking['current_lat'] && $tracking['current_lng']) ? '' : 'display:none;'; ?> height:200px; border-radius:12px; margin-top:12px;"></div>
            <p id="riderLocationNote" style="font-size:11px;color:#9ca3af;margin-top:8px;text-align:center;<?php echo ($tracking && $tracking['current_lat'] && $tracking['current_lng']) ? '' : 'display:none;'; ?>">Rider's live location</p>
        </div>
        <?php endif; ?>
...
(function() {
    ...
    var map = null, riderMarker = null;

    <?php if ($tracking && $tracking['current_lat'] && $tracking['current_lng']): ?>
    map = L.map('trackMap').setView([<?php echo $tracking['current_lat']; ?>, <?php echo $tracking['current_lng']; ?>], 15);
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { maxZoom: 19 }).addTo(map);
    document.getElementById('trackMap').style.display = '';
    ...
    riderMarker = L.marker([<?php echo $tracking['current_lat']; ?>, <?php echo $tracking['current_lng']; ?>], { icon: riderIcon })
        .addTo(map).bindPopup('Rider is here');
    <?php endif; ?>
        ...
        fetch('../api/get_tracking_status.php?order_number=<?php echo urlencode($order['order_number']); ?>&customer_phone=<?php echo urlencode($customerPhone); ?>')
            ...
            .then(function(d) {
                ...
                if (d.tracking) {
                    ...
                    if (d.tracking.current_lat && d.tracking.current_lng && riderMarker) {
                        riderMarker.setLatLng([d.tracking.current_lat, d.tracking.current_lng]);
                        if (map) map.panTo([d.tracking.current_lat, d.tracking.current_lng], { animate: true });
                    }
                }
            })
            .catch(function() {});
